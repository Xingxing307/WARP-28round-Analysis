...
# 初始化统计变量
total_even_count_after = 0  # 累计置换后偶数位置的1的总个数
...
for round_num in range(1, num_rounds + 1):
    ...
    # 统计当前轮次中置换后1在偶数位置的个数
    even_count_after = sum(1 for pos in new_ones_positions if pos % 2 == 0)  # 统计偶数位置
    total_even_count_after += even_count_after  # 累加到总偶数计数
    ...
    for pos in new_ones_positions:
        if pos % 2 == 0:  # 偶数
            next_ones_positions.append(pos)  # 加入偶数本身
            if pos + 1 <= 31:  # 保证加1后的数字不超出范围
                next_ones_positions.append(pos + 1)  # 加入加1后的数字
        else:  # 奇数
            next_ones_positions.append(pos)  # 直接加入奇数位置
    ...
    print(f"当前轮次置换后1在偶数位置的个数: {even_count_after}")
    print(f"累计置换后1在偶数位置的总个数: {total_even_count_after}")
    print()
